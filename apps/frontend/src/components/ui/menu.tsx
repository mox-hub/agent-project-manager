...
/*
 * coss ui Menu（base-ui 配方移植，registry @coss/menu）
 * 下拉菜单标准件；右键菜单（context-menu.tsx）复用本文件的条目样式常量。
 */
...
/** 条目/弹出层样式常量（供 context-menu 等 非 MenuRoot 容器复用同一设计） */
  ...
  return (
    <MenuPrimitive.Item
      className={cn(
        ...
        "data-inset:ps-8 data-[variant=destructive]:text-destructive data-[variant=destructive]:data-highlighted:bg-destructive/10 data-[variant=destructive]:data-highlighted:text-destructive",
        className
      )}
      data-inset={inset}
      data-slot="menu-item"
...
export function MenuCheckboxItem({
  ...
  return (
    <MenuPrimitive.CheckboxItem
      checked={checked}
      className={cn(MENU_CHECKBOX_ITEM_CLASS, className)}
  ...
  return (
    <MenuPrimitive.RadioItem
      className={cn(
        ...
        "cursor-default grid-cols-[.75rem_1fr] items-center",
        className
      )}
