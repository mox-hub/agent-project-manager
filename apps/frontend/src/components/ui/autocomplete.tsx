...
/*
 * coss ui Autocomplete（base-ui 配方移植，registry @coss/autocomplete）
 * 适配：Input 无 nativeInput/size 视觉 prop，尺寸用高度 class 映射。
 */

/** size → 高度 class（对齐 Input 的 h-8/h-9/h-10 三档） */
...
export function AutocompleteInput({
  ...
  const heightClass =
    SIZE_HEIGHT[size] ?? (typeof size === "number" ? undefined : undefined)
  ...
  return (
    ...
    >
      ...
      {showClear && (
        <AutocompleteClear
          className={cn(
            "absolute top-1/2 inline-flex size-8 shrink-0 -translate-y-1/2 cursor-pointer items-center justify-center rounded-md border border-transparent opacity-80 outline-none transition-[color,background-color,box-shadow,opacity] pointer-coarse:after:absolute pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 hover:opacity-100 sm:size-7 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
            size === "sm" ? "end-0" : "end-0.5"
          )}
          {...clearProps}
        >
