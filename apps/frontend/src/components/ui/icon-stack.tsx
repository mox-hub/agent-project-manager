...
/**
 * 等距层叠图标插画容器（reui base-nova 配方移植，2026-09-14）。
 * 装饰性组件：用于空态/引导/完成时刻的深度感图标——children 居中渲染在层叠卡堆上，
 * 尺寸经 className 控制（默认 h-20 w-18），语义色经 text-* 传入。
 * 纯装饰场景记得 aria-hidden="true"，可读文案放在周边内容里。
 * 与 reui 原配方差异：椭圆投影模糊 token 化（blur-xs=4px）、内容定位改内联 style（避开任意值类）。
 */
    ...
    >
      ...
      {children ? (
        <div
          ...
          className="text-muted-foreground pointer-events-none absolute flex -translate-x-1/2 -translate-y-1/2 scale-x-90 -skew-y-26 items-center justify-center"
          style={{
            top: 'var(--icon-stack-content-y)',
            left: 'var(--icon-stack-content-x)',
          }}
        >
          {children}
        </div>
