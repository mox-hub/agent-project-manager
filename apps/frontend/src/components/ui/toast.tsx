 ...
 * 底层为 base-ui Toast + createToastManager；命令式 API 见文件尾部 sonner 兼容层。
 */
...
/* ============================================
   sonner 兼容层（历史 API，2026-08 sonner → coss toast 迁移保留）
   toast.success/error/info/warning/loading/dismiss/promise 命令式签名不变，
   调用点只改 import 来源（'sonner' → '@/components/ui/toast'）。
   ============================================ */

export interface ExternalToast {
  description?: React.ReactNode
  /** 自动关闭延时 ms；Infinity 表示常驻 */
  duration?: number
  id?: string | number
...
type ToastFn = {
  (message: React.ReactNode, opts?: ExternalToast): string
  success: (message: React.ReactNode, opts?: ExternalToast) => string
  error: (message: React.ReactNode, opts?: ExternalToast) => string
  info: (message: React.ReactNode, opts?: ExternalToast) => string
  warning: (message: React.ReactNode, opts?: ExternalToast) => string
  loading: (message: React.ReactNode, opts?: ExternalToast) => string
...
const toast = ((message: React.ReactNode, opts?: ExternalToast) =>
  enqueue(undefined, message, opts)) as ToastFn

toast.success = (message, opts) => enqueue("success", message, opts)
toast.error = (message, opts) => enqueue("error", message, opts)
toast.info = (message, opts) => enqueue("info", message, opts)
toast.warning = (message, opts) => enqueue("warning", message, opts)
toast.loading = (message, opts) => enqueue("loading", message, opts)
toast.dismiss = (id) => toastManager.close(id != null ? String(id) : undefined)
toast.promise = <Value,>(
  promiseValue: Promise<Value>,
  opts: {
    loading: string
  ...
  toastManager.promise(promiseValue, {
    loading: { title: opts.loading, type: "loading" },
    success: (result) => ({
      title:
    ...
    error: (error) => ({
      title: typeof opts.error === "function" ? opts.error(error) : opts.error,
      type: "error",
    }),
  })

export { toast, Toast as ToastPrimitive }
