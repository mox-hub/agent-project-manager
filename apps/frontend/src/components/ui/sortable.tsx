...
/**
 * 复合式拖拽排序列表（reui base-nova 配方移植，2026-09-14）。
 * 语义限定「同列表条目重排序」——看板跨列拖拽/画布节点/文件投放请继续用 dnd-kit 原语。
 * 落放一次性提交：onValueChange 收到重排后全量数组即可持久化；
 * 需要索引式变更与回滚快照时用 onValueCommit（携带 previousValue）。
 */
...
/**
 * createPortal 的客户端挂载闸（portal 需要 document.body，SSR/水合期不可执行）。
 * 永不通知的订阅让 useSyncExternalStore 在服务端返回 false、挂载后返回 true，
 * 等价于旧版 useLayoutEffect(() => setMounted(true), []) 且少一次渲染，
 * 同时避开 react-hooks/set-state-in-effect 规则。三个函数都在模块作用域，
 * 保证订阅身份稳定（内联 getSnapshot 是无限重订阅的经典成因）。
 */
