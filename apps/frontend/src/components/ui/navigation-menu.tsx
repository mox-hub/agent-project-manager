...
function NavigationMenuLink({ children, ...props }: React.ComponentProps<"a">) {
  return <a className="block select-none rounded-md px-3 py-2 no-underline outline-hidden transition-colors hover:bg-accent hover:text-accent-foreground" {...props}>{children}</a>
}
