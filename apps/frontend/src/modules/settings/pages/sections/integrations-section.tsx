...
function LogoBadge({ logo, color, size = 'md' }: { logo: string; color: string; size?: 'sm' | 'md' | 'lg' }) {
  const dim = size === 'lg' ? 'w-12 h-12 text-base' : size === 'sm' ? 'w-7 h-7 text-[10px]' : 'w-10 h-10 text-xs';
  return (
    <div
...
function StatusBadge({ status }: { status: ConnectionStatus }) {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-0.5 rounded-full border', cfg.bg, cfg.color)}>
      <span
        className={cn(
...
function FeatureToggle({ feature }: { feature: IntegrationFeature }) {
  ...
  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-border/60 last:border-0">
      ...
      <div
        className={cn(
          'relative shrink-0 w-8 h-[18px] rounded-full border transition-all duration-200 mt-0.5',
      ...
      >
        <span
          className={cn(
            ...
            feature.enabled ? 'left-[calc(100%-15px)] bg-white' : 'left-[2px] bg-muted-foreground/40',
          )}
        />
      </div>
    </div>
        ...
        <div className="border-t border-border/60 px-5 pb-5 pt-4">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-4">
            {integration.features.slice(0, 4).map((f) => {
              const Icon = f.icon;
              return (
                <div key={f.label} className="flex items-center gap-2 text-[11px] text-muted-foreground/60">
            ...
            {integration.features.length > 4 && (
              <div className="text-[11px] text-muted-foreground/40">+{integration.features.length - 4} more features</div>
            )}
          </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {[
                  ...
                  >
                    ...
                    <span className="ml-auto text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground shrink-0">Soon</span>
                  </div>
                ))}
              </div>
            </section>
