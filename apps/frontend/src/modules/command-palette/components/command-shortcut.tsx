    ...
    <div className="flex shrink-0 items-center gap-0.5">
      {shortcut.map((key, index) => (
        <React.Fragment key={index}>
          <kbd className="inline-flex h-5 min-w-[1.25rem] items-center justify-center rounded border border-border/50 bg-muted/50 px-1 text-[10px] font-medium text-muted-foreground shadow-sm">
            {formatKey(key)}
          </kbd>
