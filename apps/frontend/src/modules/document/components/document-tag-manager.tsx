...
export function DocumentTagManager({ documentId }: DocumentTagManagerProps) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-start gap-2 rounded-md border border-dashed border-accent-blue/30 bg-accent-blue/5 px-2.5 py-2 text-[11px] leading-relaxed text-muted-foreground">
        <Info size={12} className="mt-0.5 shrink-0 text-accent-blue" />
        <span>
          标签以 Markdown 文件的 <code className="rounded bg-muted px-1 font-mono text-[10px]">frontmatter</code> 为准,
          编辑文档源文件的 <code className="rounded bg-muted px-1 font-mono text-[10px]">tags: [a, b]</code> 来管理。保存后会自动同步。
        </span>
      </div>
