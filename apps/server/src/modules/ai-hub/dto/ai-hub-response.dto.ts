...
/** GET /ai/workflows/:id 返回（AIWorkflowDefinition） */
...
/** POST /ai/workflows/:id/run 返回 */
export class RunWorkflowResponseDto {
  @ApiProperty({ type: String, description: '工作流运行 ID' })
  workflowRunId: string;

  @ApiProperty({ type: String, description: '运行状态（创建后初始 pending）' })
  status: string;
}
...
export class WorkflowRunDto {
  @ApiProperty({ type: String }) id: string;
  @ApiProperty({ type: String, description: '工作流 ID' })
  workflowId: string;
  @ApiPropertyOptional({ type: String, nullable: true, description: '项目 ID' })
  projectId?: string | null;
  @ApiPropertyOptional({ type: String, nullable: true, description: '任务 ID' })
  issueId?: string | null;
...
/** GET /ai/workflow-runs 返回（{ data, meta } 分页口径） */
