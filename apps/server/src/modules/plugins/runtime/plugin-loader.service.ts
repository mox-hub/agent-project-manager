...
import { Plugin, PluginPermission, PluginScope } from '@prisma/client';
...
export class PluginLoaderService {
  ...
  async loadPluginFromPath(path: string): Promise<any> {
    try {
      ...
      const fs = await import('fs').then(fs => fs.promises.readFile(path, 'utf-8'));
      return JSON.parse(fs.toString());
    } catch (error) {
      throw new Error(`Failed to load plugin manifest from ${path}: ${error.message}`);
    }
  }
  ...
  async validateManifest(manifest: any): Promise<{ valid: boolean; errors: string[] }> {
    ...
    const requiredManifestKeys = ['name', 'type', 'entry', 'file', 'permissions', 'dependencies'];
    for (const entry of manifest.entries || []) {
      ...
      if (!entry.type) {
        errors.push(`Manifest entry "${entry.name}" must specify a type (component, provider, theme)`);
      }
      ...
      for (const perm of entry.permissions || []) {
        if (!perm.permission) {
          errors.push(`Permission for "${entry.name}" must specify a permission string`);
        }
      }
    }
  ...
  async loadPluginCode(plugin: Plugin): Promise<any> {
    ...
    return {
      loaded: true,
      context: {},
      entryPoint: plugin.manifest.main || 'index.js',
    };
  }
   ...
   */
  async validatePlugin(plugin: Plugin): Promise<{ valid: boolean; errors: string[] }> {
    ...
    if (grantedPermissions.length === 0) {
      manifestErrors.push(`Plugin "${plugin.name}" requires at least one granted permission`);
    }
