...
describe('Task Templates (e2e)', () => {
  ...
  describe('POST /_api/issue-templates', () => {
    it('should create a template with items', () => {
      return wsHttp
        .post('/_api/issue-templates')
  ...
  describe('GET /_api/issue-templates', () => {
    it('should list templates', () => {
      return wsHttp
        .get('/_api/issue-templates')
        .set('Authorization', `Bearer ${accessToken}`)
        .expect(200)
        .expect((res: Response) => {
  ...
  describe('GET /_api/issue-templates/:id', () => {
    it('should get template detail', () => {
      return wsHttp
        .get(`/_api/issue-templates/${templateId}`)
        .set('Authorization', `Bearer ${accessToken}`)
        .expect(200)
        .expect((res: Response) => {
    ...
    it('should 404 on unknown id', () => {
      return wsHttp
        .get('/_api/issue-templates/nonexistent-template')
        .set('Authorization', `Bearer ${accessToken}`)
        .expect(404);
    });
  });

  describe('PATCH /_api/issue-templates/:id', () => {
    it('should update template', () => {
      return wsHttp
        .patch(`/_api/issue-templates/${templateId}`)
        .set('Authorization', `Bearer ${accessToken}`)
  ...
  describe('POST /_api/issue-templates/:id/use', () => {
    it('should instantiate tasks from template', () => {
      return wsHttp
        .post(`/_api/issue-templates/${templateId}/use`)
        .set('Authorization', `Bearer ${accessToken}`)
        .send({ projectId })
        .expect(201)
        .expect((res: Response) => {
  ...
  describe('DELETE /_api/issue-templates/:id', () => {
    it('should delete template', () => {
      return wsHttp
        .delete(`/_api/issue-templates/${templateId}`)
        .set('Authorization', `Bearer ${accessToken}`)
        .expect(200);
    });
  });
